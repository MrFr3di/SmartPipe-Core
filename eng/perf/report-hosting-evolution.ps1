...
if ([string]$runManifest.scenarioClass -cne 'evolution') {
    throw "Hosting report expects scenarioClass=evolution, got '$($runManifest.scenarioClass)'."
}
...
if ($records.Count -eq 0) {
    throw 'No BenchmarkDotNet Hosting records were found.'
}
...
$lines.Add('# Hosting evolution comparison')
$lines.Add('')
$lines.Add("Run: $($runManifest.runId)")
$lines.Add('')
$lines.Add('This is an evolution comparison, not a strict A/B regression test. SmartPipe 2.2.0 uses one keyed multi-pipeline hosted orchestrator with ordered startup, reverse shutdown, rollback, and per-run scope ownership; SmartPipe 2.1.2 uses one generic hosted service per pipeline. Cross-version percentage deltas are intentionally omitted.')
$lines.Add('')
...
Write-Output "PERF_HOSTING_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
