...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-healthchecks-evolution.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/health-checks/$runId"
...
foreach ($entry in $order) {
    ...
    $arguments = @(
        ...
        $Job,
        '--filter',
        '*HealthChecksEvolutionBenchmarks*',
        '--artifacts',
        $artifactDir,
        '--exporters',
        'json',
    ...
    if ($exitCode -ne 0) {
        throw "HealthChecks evolution slot $slot ($target) failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "HealthChecks evolution slot $slot ($target)"
}
...
Write-Output "PERF_HEALTHCHECKS_EVOLUTION_RUN_OK runId=$runId job=$Job order=v212,v220,v220,v212"
