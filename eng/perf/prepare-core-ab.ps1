...
function Get-NuGetContentHash {
    ...
    try {
        ...
        $line = $output | Where-Object { $_ -match '^Content hash:\s*(?<hash>\S+)\s*
...
function Prepare-BenchmarkTarget {
    ...
    Assert-RestoredPackageSource -PackageId 'SmartPipe.Core' -PackageVersion $ExpectedPackageVersion -ExpectedFeed $packagesDir
...
Write-Output "PERF_CORE_AB_READY baseline=2.1.2 candidate=$CandidateSha sourceSha256=$sharedSourceSha"
        ...
        if ([string]$line -notmatch '^Content hash:\s*(?<hash>\S+)\s*
function New-NuGetConfig {
    param(
        [string]$Path,
        [string]$LocalFeed
...
function Assert-RestoredPackageSource {
    ...
    $packageRoot = Join-Path $env:NUGET_PACKAGES ($PackageId.ToLowerInvariant() + '/' + $PackageVersion.ToLowerInvariant())
