...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-opentelemetry-evolution.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/opentelemetry/$runId"
...
foreach ($entry in $order) {
    ...
    if ($exitCode -ne 0) {
        throw "OpenTelemetry evolution slot $slot ($target) failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "OpenTelemetry evolution slot $slot ($target)"
}
