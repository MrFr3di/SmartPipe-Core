...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for OpenTelemetry provenance verification.'
    }
...
function Prepare-OpenTelemetryTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate OpenTelemetry lock file for $TargetId"
    ...
    Assert-ExitCode "Locked OpenTelemetry restore for $TargetId"
    ...
    Assert-ExitCode "Build OpenTelemetry evolution target $TargetId"
    ...
    if ($RunDry) {
        ...
        $benchmarkArgs = @(
            ...
            '--no-build', '--',
            '--job', 'Dry',
            '--filter', '*OpenTelemetryEvolutionBenchmarks*',
        ...
        Assert-ExitCode "BenchmarkDotNet OpenTelemetry Dry $TargetId"
        Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step "BenchmarkDotNet OpenTelemetry Dry $TargetId"
    }

    Write-Output "PERF_OPENTELEMETRY_TARGET_READY target=$TargetId packages=$($smartPipeEntries.Count)"
}
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.OpenTelemetry.Shared/OpenTelemetryEvolutionBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/opentelemetry'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.OpenTelemetry.V212/SmartPipe.Perf.OpenTelemetry.V212.csproj'
$baselineAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.OpenTelemetry.V212/OpenTelemetryTarget.cs'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.OpenTelemetry.V220/SmartPipe.Perf.OpenTelemetry.V220.csproj'
$candidateAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.OpenTelemetry.V220/OpenTelemetryTarget.cs'

Prepare-OpenTelemetryTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineAdapter -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Core' -ExpectedPrimaryVersion '2.1.2'
Prepare-OpenTelemetryTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateAdapter -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.OpenTelemetry' -ExpectedPrimaryVersion '2.2.0'

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_OPENTELEMETRY_EVOLUTION_READY baseline=2.1.2 candidate=$CandidateSha sharedSourceSha256=$sharedSourceSha"
