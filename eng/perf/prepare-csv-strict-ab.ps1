...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for CsvStrictAb provenance verification.'
    }
...
function Prepare-CsvStrictAbTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate CsvStrictAb lock file for $TargetId"
    ...
    Assert-ExitCode "Locked CsvStrictAb restore for $TargetId"
    ...
    Assert-ExitCode "Build CsvStrictAb evolution target $TargetId"
    ...
    if ($RunDry) {
        ...
        Assert-ExitCode "BenchmarkDotNet CsvStrictAb Dry $TargetId"
        Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step "BenchmarkDotNet CsvStrictAb Dry $TargetId"
    }

    Write-Output "PERF_CSV_STRICT_AB_TARGET_READY target=$TargetId packages=$($smartPipeEntries.Count)"
}
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.CsvStrictAb.Shared/CsvStrictAbBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/csv-strict-ab'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.CsvStrictAb.V212/SmartPipe.Perf.CsvStrictAb.V212.csproj'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.CsvStrictAb.V220/SmartPipe.Perf.CsvStrictAb.V220.csproj'

Prepare-CsvStrictAbTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineProject -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Extensions' -ExpectedPrimaryVersion '2.1.2'
Prepare-CsvStrictAbTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateProject -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.Csv' -ExpectedPrimaryVersion '2.2.0'

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_CSV_STRICT_AB_READY baseline=2.1.2 candidate=$CandidateSha sharedSourceSha256=$sharedSourceSha"
