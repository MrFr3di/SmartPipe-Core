...
if ([string]$runManifest.scenarioClass -cne 'evolution') {
    throw "Dapper report expects scenarioClass=evolution, got '$($runManifest.scenarioClass)'."
}
...
foreach ($slotDirectory in $slotDirectories) {
    ...
    foreach ($benchmark in @($document.Benchmarks)) {
        ...
        if ($null -eq $statisticsProperty -or $null -eq $statisticsProperty.Value) {
            throw "Dapper result '$($benchmark.Method)' in slot $slot has no statistics."
        }
...
if ($records.Count -eq 0) {
    throw 'No BenchmarkDotNet Dapper records were found.'
}
...
$lines.Add('# Dapper evolution comparison')
$lines.Add('')
$lines.Add("Run: $($runManifest.runId)")
$lines.Add('')
$lines.Add('This is an evolution comparison, not a strict A/B regression test. SmartPipe 2.1.2 uses the legacy DapperSelector path, while SmartPipe 2.2.0 uses the new runtime-owned Dapper query source through PipelineDefinition.StartAsync and PipelineRun. Both sides read the same deterministic named in-memory SQLite database with explicit row mapping and fresh per-run connections. Cross-version percentage deltas are intentionally omitted.')
$lines.Add('')
...
Write-Output "PERF_DAPPER_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
