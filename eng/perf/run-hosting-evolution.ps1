...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-hosting-evolution.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/hosting/$runId"
...
foreach ($entry in $order) {
    ...
    if ($exitCode -ne 0) {
        throw "Hosting evolution slot $slot ($target) failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "Hosting evolution slot $slot ($target)"
}
...
Write-Output "PERF_HOSTING_EVOLUTION_RUN_OK runId=$runId job=$Job order=v212,v220,v220,v212"
