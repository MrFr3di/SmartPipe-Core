...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for EntityFrameworkCore provenance verification.'
    }
...
function Prepare-EntityFrameworkCoreTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate EntityFrameworkCore lock file for $TargetId"
    ...
    Assert-ExitCode "Locked EntityFrameworkCore restore for $TargetId"
    ...
    Assert-ExitCode "Build EntityFrameworkCore evolution target $TargetId"
    ...
    if ($RunDry) {
        ...
        $benchmarkArgs = @(
            ...
            '--no-build', '--',
            '--job', 'Dry',
            '--filter', '*EntityFrameworkCoreEvolutionBenchmarks*',
        ...
        Assert-ExitCode "BenchmarkDotNet EntityFrameworkCore Dry $TargetId"
        Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step "BenchmarkDotNet EntityFrameworkCore Dry $TargetId"
    }

    Write-Output "PERF_EFCORE_TARGET_READY target=$TargetId packages=$($smartPipeEntries.Count)"
}
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.EntityFrameworkCore.Shared/EntityFrameworkCoreEvolutionBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/entity-framework-core'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.EntityFrameworkCore.V212/SmartPipe.Perf.EntityFrameworkCore.V212.csproj'
$baselineAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.EntityFrameworkCore.V212/EntityFrameworkCoreEvolutionTarget.cs'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.EntityFrameworkCore.V220/SmartPipe.Perf.EntityFrameworkCore.V220.csproj'
$candidateAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.EntityFrameworkCore.V220/EntityFrameworkCoreEvolutionTarget.cs'

Prepare-EntityFrameworkCoreTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineAdapter -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Extensions' -ExpectedPrimaryVersion '2.1.2'
Prepare-EntityFrameworkCoreTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateAdapter -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.EntityFrameworkCore' -ExpectedPrimaryVersion '2.2.0'

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_EFCORE_EVOLUTION_READY baseline=2.1.2 candidate=$CandidateSha sharedSourceSha256=$sharedSourceSha"
