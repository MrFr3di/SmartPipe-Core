...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for ExtensionsStrictAb provenance verification.'
    }
...
function Prepare-ExtensionsStrictAbTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate ExtensionsStrictAb lock file for $TargetId"
    ...
    Assert-ExitCode "Locked ExtensionsStrictAb restore for $TargetId"
    ...
    Assert-ExitCode "Build ExtensionsStrictAb evolution target $TargetId"
    ...
    $metadata = [ordered]@{
        ...
        targetProjectSha256 = (Get-FileHash -LiteralPath $AdapterSource -Algorithm SHA256).Hash.ToLowerInvariant()
    ...
    if ($RunDry) {
        ...
        $benchmarkArgs = @(
            ...
            '--no-build', '--',
            '--job', 'Dry',
            '--filter', '*ExtensionsStrictAbBenchmarks*',
        ...
        Assert-ExitCode "BenchmarkDotNet ExtensionsStrictAb Dry $TargetId"
        Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step "BenchmarkDotNet ExtensionsStrictAb Dry $TargetId"
    }

    Write-Output "PERF_EXTENSIONS_STRICT_AB_TARGET_READY target=$TargetId packages=$($smartPipeEntries.Count)"
}
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.ExtensionsStrictAb.Shared/ExtensionsStrictAbBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/extensions-strict-ab'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.ExtensionsStrictAb.V212/SmartPipe.Perf.ExtensionsStrictAb.V212.csproj'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.ExtensionsStrictAb.V220/SmartPipe.Perf.ExtensionsStrictAb.V220.csproj'

Prepare-ExtensionsStrictAbTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineProject -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Extensions' -ExpectedPrimaryVersion '2.1.2'
Prepare-ExtensionsStrictAbTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateProject -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.Transforms' -ExpectedPrimaryVersion '2.2.0'

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_EXTENSIONS_STRICT_AB_READY baseline=2.1.2 candidate=$CandidateSha sharedSourceSha256=$sharedSourceSha"
