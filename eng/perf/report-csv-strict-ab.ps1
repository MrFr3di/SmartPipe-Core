...
if ([string]$runManifest.scenarioClass -cne 'strict-ab') {
    throw "SP220-09 CSV report expects scenarioClass=strict-ab, got '$($runManifest.scenarioClass)'."
}
...
if ($slotDirectories.Count -ne 4) {
    throw "SP220-09 CSV report requires exactly four A-B-B-A slot directories, got $($slotDirectories.Count)."
}
...
$lines.Add('# SP220-09 CSV strict A/B comparison')
$lines.Add('')
...
Write-Output "PERF_CSV_STRICT_AB_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
