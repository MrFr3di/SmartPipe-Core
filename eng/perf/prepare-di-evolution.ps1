...
function Assert-BenchmarkResult {
    ...
    if ($benchmarks.Count -eq 0) {
        throw "$Step produced an empty BenchmarkDotNet result set."
    }
...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for DI provenance verification.'
    }
...
function Prepare-DiTarget {
    ...
    Assert-ExitCode "Generate DI lock file for $TargetId"
    ...
    Assert-ExitCode "Locked DI restore for $TargetId"
    ...
    Assert-ExitCode "Build DI evolution target $TargetId"
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.DependencyInjection.Shared/DependencyInjectionEvolutionBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/dependency-injection'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.DependencyInjection.V212/SmartPipe.Perf.DependencyInjection.V212.csproj'
$baselineAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.DependencyInjection.V212/DependencyInjectionTarget.cs'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.DependencyInjection.V220/SmartPipe.Perf.DependencyInjection.V220.csproj'
$candidateAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.DependencyInjection.V220/DependencyInjectionTarget.cs'
