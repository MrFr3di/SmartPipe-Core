...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-extensions-strict-ab.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/extensions-strict-ab/$runId"
...
foreach ($entry in $order) {
    ...
    if ($exitCode -ne 0) {
        throw "SP220-07 strict A/B slot $slot ($target) failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "SP220-07 strict A/B slot $slot ($target)"
}
...
Write-Output "PERF_EXTENSIONS_STRICT_AB_RUN_OK runId=$runId job=$Job order=v212,v220,v220,v212"
