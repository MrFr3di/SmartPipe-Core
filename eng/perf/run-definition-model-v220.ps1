...
function Assert-BenchmarkResult {
    param([string]$Artifacts, [string]$Step)
...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-definition-model-v220.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/definition-model/$runId"
...
foreach ($slot in 1..2) {
    ...
    $arguments = @(
        ...
        '--',
        '--job', $Job,
        '--filter', '*DefinitionModelBenchmarks*',
    ...
    if ($exitCode -ne 0) {
        throw "Definition-model v2.2 slot $slot failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "Definition-model v2.2 slot $slot"
}
...
$manifest = [ordered]@{
    ...
    benchmarkSourceSha256 = $sourceSha
...
Write-Output "PERF_DEFINITION_MODEL_V220_RUN_OK runId=$runId job=$Job repeats=2"
