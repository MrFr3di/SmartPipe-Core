...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-entity-framework-core-evolution.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/entity-framework-core/$runId"
...
foreach ($entry in $order) {
    ...
    if ($exitCode -ne 0) {
        throw "EntityFrameworkCore evolution slot $slot ($target) failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "EntityFrameworkCore evolution slot $slot ($target)"
}
...
Write-Output "PERF_EFCORE_EVOLUTION_RUN_OK runId=$runId job=$Job order=v212,v220,v220,v212"
