...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for JsonStrictAb provenance verification.'
    }
...
function Prepare-JsonStrictAbTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate JsonStrictAb lock file for $TargetId"
    ...
    Assert-ExitCode "Locked JsonStrictAb restore for $TargetId"
    ...
    Assert-ExitCode "Build JsonStrictAb evolution target $TargetId"
    ...
    if ($RunDry) {
        ...
        Assert-ExitCode "BenchmarkDotNet JsonStrictAb Dry $TargetId"
        Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step "BenchmarkDotNet JsonStrictAb Dry $TargetId"
    }

    Write-Output "PERF_JSON_STRICT_AB_TARGET_READY target=$TargetId packages=$($smartPipeEntries.Count)"
}
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.JsonStrictAb.Shared/JsonStrictAbBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/json-strict-ab'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.JsonStrictAb.V212/SmartPipe.Perf.JsonStrictAb.V212.csproj'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.JsonStrictAb.V220/SmartPipe.Perf.JsonStrictAb.V220.csproj'

Prepare-JsonStrictAbTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineProject -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Extensions.Json' -ExpectedPrimaryVersion '2.1.2'
Prepare-JsonStrictAbTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateProject -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.Json' -ExpectedPrimaryVersion '2.2.0'

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_JSON_STRICT_AB_READY baseline=2.1.2 candidate=$CandidateSha sharedSourceSha256=$sharedSourceSha"
