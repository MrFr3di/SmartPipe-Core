...
if ([string]$runManifest.scenarioClass -cne 'evolution') {
    throw "DI report expects scenarioClass=evolution, got '$($runManifest.scenarioClass)'."
}
...
if ($records.Count -eq 0) {
    throw 'No BenchmarkDotNet DI records were found.'
}
...
$lines.Add('# Dependency Injection evolution comparison')
$lines.Add('')
$lines.Add("Run: $($runManifest.runId)")
$lines.Add('')
$lines.Add('This is an evolution comparison, not a strict A/B regression test. SmartPipe 2.2.0 uses keyed registration, a run registry/observation store, and per-run scope ownership that are not equivalent to the 2.1.2 DI implementation. Cross-version percentage deltas are intentionally omitted.')
$lines.Add('')
...
Write-Output "PERF_DI_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
