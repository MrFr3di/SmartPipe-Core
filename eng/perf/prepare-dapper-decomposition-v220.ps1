...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for dapper-decomposition provenance verification.'
    }
...
$project = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.Dapper.Decomposition.V220/SmartPipe.Perf.Dapper.Decomposition.V220.csproj'
$source = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.Dapper.Decomposition.V220/DapperDecompositionBenchmarks.cs'
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/dapper-decomposition/v220'
...
Assert-ExitCode 'Generate dapper-decomposition lock file'
...
$primaryEntry = $smartPipeEntries |
    Where-Object { $_.Name -ceq 'SmartPipe.Extensions.Dapper' } |
    Select-Object -First 1

if ($null -eq $primaryEntry -or [string]$primaryEntry.Value.resolved -cne '2.2.0') {
    throw 'Dapper decomposition target must resolve SmartPipe.Extensions.Dapper 2.2.0 from the candidate feed.'
}
...
foreach ($entry in $smartPipeEntries) {
    ...
    if ([string]$entry.Value.contentHash -cne $nugetContentHash) {
        throw "Dapper decomposition lock contentHash mismatch for '$packageId/$packageVersion'."
    }
...
Assert-ExitCode 'Locked dapper-decomposition restore'
...
Assert-ExitCode 'Build dapper-decomposition v2.2-only target'
...
[ordered]@{
    ...
    primaryPackageId = 'SmartPipe.Extensions.Dapper'
...
if ($RunDry) {
    ...
    Assert-ExitCode 'BenchmarkDotNet dapper-decomposition Dry'
    Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step 'BenchmarkDotNet dapper-decomposition Dry'
}

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_DAPPER_DECOMPOSITION_V220_READY candidate=$CandidateSha methods=$($expectedMethods.Count)"
