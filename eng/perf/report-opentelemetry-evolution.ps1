...
if ([string]$runManifest.scenarioClass -cne 'evolution') {
    throw "OpenTelemetry report expects scenarioClass=evolution, got '$($runManifest.scenarioClass)'."
}
...
foreach ($slotDirectory in $slotDirectories) {
    ...
    foreach ($benchmark in @($document.Benchmarks)) {
        ...
        if ($null -eq $statisticsProperty -or $null -eq $statisticsProperty.Value) {
            throw "OpenTelemetry result '$($benchmark.Method)' in slot $slot has no statistics."
        }
...
if ($records.Count -eq 0) {
    throw 'No BenchmarkDotNet OpenTelemetry records were found.'
}
...
$lines.Add('# OpenTelemetry evolution comparison')
$lines.Add('')
$lines.Add("Run: $($runManifest.runId)")
$lines.Add('')
$lines.Add('This is an evolution comparison, not a strict A/B regression test. SmartPipe 2.1.2 is represented by equivalent application-owned manual AddMeter/AddSource registration, while SmartPipe 2.2.0 uses AddSmartPipeInstrumentation. Both sides use the same OpenTelemetry provider stack and no exporter in the timed benchmark region. Cross-version percentage deltas are intentionally omitted.')
$lines.Add('')
...
Write-Output "PERF_OPENTELEMETRY_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
