...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-extensions-focus.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/extensions-focus/$runId"
...
foreach ($entry in $order) {
    ...
    if ($exitCode -ne 0) {
        throw "SP220-07 focused Composite strict A/B slot $slot ($target) failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "SP220-07 focused Composite strict A/B slot $slot ($target)"
}
...
Write-Output "PERF_EXTENSIONS_FOCUS_RUN_OK runId=$runId job=$Job order=v212,v220,v220,v212"
