...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for HealthChecks provenance verification.'
    }
...
function Prepare-HealthChecksTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate HealthChecks lock file for $TargetId"
    ...
    Assert-ExitCode "Locked HealthChecks restore for $TargetId"
    ...
    Assert-ExitCode "Build HealthChecks evolution target $TargetId"
    ...
    if ($RunDry) {
        ...
        $benchmarkArgs = @(
            ...
            '--no-build', '--',
            '--job', 'Dry',
            '--filter', '*HealthChecksEvolutionBenchmarks*',
        ...
        Assert-ExitCode "BenchmarkDotNet HealthChecks Dry $TargetId"
...
$sharedSource = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.HealthChecks.Shared/HealthChecksEvolutionBenchmarks.cs'
$sharedSourceSha = (Get-FileHash -LiteralPath $sharedSource -Algorithm SHA256).Hash.ToLowerInvariant()
$artifactsRoot = Join-Path $repoRoot 'artifacts/perf/health-checks'

New-Item -ItemType Directory -Path $artifactsRoot -Force | Out-Null

$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.HealthChecks.V212/SmartPipe.Perf.HealthChecks.V212.csproj'
$baselineAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.HealthChecks.V212/HealthChecksTarget.cs'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.HealthChecks.V220/SmartPipe.Perf.HealthChecks.V220.csproj'
$candidateAdapter = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.HealthChecks.V220/HealthChecksTarget.cs'

Prepare-HealthChecksTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineAdapter -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Extensions' -ExpectedPrimaryVersion '2.1.2'
Prepare-HealthChecksTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateAdapter -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.HealthChecks' -ExpectedPrimaryVersion '2.2.0'
