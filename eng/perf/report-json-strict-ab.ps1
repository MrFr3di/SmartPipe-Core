...
if ([string]$runManifest.scenarioClass -cne 'strict-ab') {
    throw "SP220-08 JSON report expects scenarioClass=strict-ab, got '$($runManifest.scenarioClass)'."
}
...
if ($slotDirectories.Count -ne 4) {
    throw "SP220-08 JSON report requires exactly four A-B-B-A slot directories, got $($slotDirectories.Count)."
}
...
$lines.Add('# SP220-08 JSON strict A/B comparison')
$lines.Add('')
...
Write-Output "PERF_JSON_STRICT_AB_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
