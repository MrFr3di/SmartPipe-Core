...
$prepare = Join-Path $repoRoot 'eng/perf/prepare-dapper-decomposition-v220.ps1'
...
$runRoot = Join-Path $repoRoot "artifacts/perf/runs/dapper-decomposition/$runId"
...
foreach ($slot in 1..2) {
    ...
    $arguments = @(
        ...
        '--',
        '--job', $Job,
        '--filter', '*DapperDecompositionBenchmarks*',
    ...
    if ($exitCode -ne 0) {
        throw "Dapper decomposition slot $slot failed with exit code $exitCode."
    }

    Assert-BenchmarkResult -Artifacts $artifactDir -Step "Dapper decomposition slot $slot"
}
...
Write-Output "PERF_DAPPER_DECOMPOSITION_RUN_OK runId=$runId job=$Job repeats=2"
