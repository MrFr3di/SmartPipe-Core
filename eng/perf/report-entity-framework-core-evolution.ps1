...
if ([string]$runManifest.scenarioClass -cne 'evolution') {
    throw "Entity Framework Core report expects scenarioClass=evolution, got '$($runManifest.scenarioClass)'."
}
...
foreach ($slotDirectory in $slotDirectories) {
    ...
    foreach ($benchmark in @($document.Benchmarks)) {
        ...
        if ($null -eq $statisticsProperty -or $null -eq $statisticsProperty.Value) {
            throw "Entity Framework Core result '$($benchmark.Method)' in slot $slot has no statistics."
        }
...
if ($records.Count -eq 0) {
    throw 'No BenchmarkDotNet Entity Framework Core records were found.'
}
...
$lines.Add('# Entity Framework Core evolution comparison')
$lines.Add('')
$lines.Add("Run: $($runManifest.runId)")
$lines.Add('')
$lines.Add('This is an evolution comparison, not a strict A/B regression test. SmartPipe 2.1.2 uses the legacy EfCoreSelector path, while SmartPipe 2.2.0 uses the new runtime-owned EF Core query source through PipelineDefinition.StartAsync and PipelineRun. Both sides use Microsoft.EntityFrameworkCore.Sqlite 10.0.11 against the same SQLite in-memory relational database, the same seeded 100-row dataset, and no-tracking query semantics. Cross-version percentage deltas are intentionally omitted.')
$lines.Add('')
...
Write-Output "PERF_EFCORE_REPORT_OK run=$($runManifest.runId) comparisons=$($comparisons.Count)"
