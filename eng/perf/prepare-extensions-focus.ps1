...
function Clear-SmartPipeGlobalPackages {
    if ([string]::IsNullOrWhiteSpace($env:NUGET_PACKAGES)) {
        throw 'NUGET_PACKAGES must be set for ExtensionsFocus provenance verification.'
    }
...
function Prepare-ExtensionsFocusTarget {
    param(
        [string]$TargetId,
        [string]$ProjectPath,
    ...
    Assert-ExitCode "Generate ExtensionsFocus lock file for $TargetId"
    ...
    Assert-ExitCode "Locked ExtensionsFocus restore for $TargetId"
    ...
    Assert-ExitCode "Build ExtensionsFocus evolution target $TargetId"
    ...
    if ($RunDry) {
        ...
        $benchmarkArgs = @(
            ...
            '--no-build', '--',
            '--job', 'Dry',
            '--filter', '*CompositeFocusBenchmarks*',
        ...
        Assert-ExitCode "BenchmarkDotNet ExtensionsFocus Dry $TargetId"
        Assert-BenchmarkResult -Artifacts $bdnArtifacts -Step "BenchmarkDotNet ExtensionsFocus Dry $TargetId"
    }

    Write-Output "PERF_EXTENSIONS_FOCUS_TARGET_READY target=$TargetId packages=$($smartPipeEntries.Count)"
}
...
$baselineProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.ExtensionsFocus.V212/SmartPipe.Perf.ExtensionsFocus.V212.csproj'
$candidateProject = Join-Path $repoRoot 'perf/src/SmartPipe.Perf.ExtensionsFocus.V220/SmartPipe.Perf.ExtensionsFocus.V220.csproj'

Prepare-ExtensionsFocusTarget -TargetId 'v212' -ProjectPath $baselineProject -AdapterSource $baselineProject -TargetRoot $baselineRoot -PrimaryPackageId 'SmartPipe.Extensions' -ExpectedPrimaryVersion '2.1.2'
Prepare-ExtensionsFocusTarget -TargetId 'v220' -ProjectPath $candidateProject -AdapterSource $candidateProject -TargetRoot $candidateRoot -PrimaryPackageId 'SmartPipe.Extensions.Transforms' -ExpectedPrimaryVersion '2.2.0'

(& dotnet --info) |
    Set-Content -LiteralPath (Join-Path $artifactsRoot 'dotnet-info.txt') -Encoding utf8

Write-Output "PERF_EXTENSIONS_FOCUS_READY baseline=2.1.2 candidate=$CandidateSha sharedSourceSha256=$sharedSourceSha"
