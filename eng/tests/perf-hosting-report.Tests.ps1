    ...
    $document = [ordered]@{
        ...
        Benchmarks = @(
            [ordered]@{
                ...
                Parameters = ''
                FullName = 'Synthetic Hosting ResolveHostedServiceGraph'
            ...
            [ordered]@{
                ...
                Parameters = ''
                FullName = 'Synthetic Hosting StartStopHostedPipeline'
                HardwareIntrinsics = 'synthetic'
                Statistics = [ordered]@{
                    N = 3
                    Mean = ($Mean * 2.0)
...
$report = Join-Path $repoRoot 'eng/perf/report-hosting-evolution.ps1'
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("smartpipe-perf-hosting-report-" + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $manifest = [ordered]@{
        schemaVersion = 1
        runId = 'synthetic-hosting-report-test'
        scenario = 'hosting'
    ...
    Assert-True (Test-Path -LiteralPath $normalizedPath -PathType Leaf) 'Hosting normalized-results.json was not created.'
    Assert-True (Test-Path -LiteralPath $markdownPath -PathType Leaf) 'Hosting comparison.md was not created.'

    $normalized = Get-Content -LiteralPath $normalizedPath -Raw | ConvertFrom-Json -Depth 64
    Assert-True ([string]$normalized.scenarioClass -ceq 'evolution') 'Hosting normalized class must remain evolution.'
    Assert-True ([string]$normalized.comparisonPolicy -ceq 'side-by-side-no-cross-version-ratio') 'Hosting comparison policy drifted.'
    Assert-True (@($normalized.records).Count -eq 8) 'Expected eight normalized Hosting raw records.'
    Assert-True (@($normalized.sideBySide).Count -eq 2) 'Expected two distinct Hosting side-by-side method groups.'
    ...
    Assert-True ([Math]::Abs([double]$comparison.baselineMeanNs - 950.0) -lt 0.001) 'Hosting baseline center is incorrect.'
    Assert-True ([Math]::Abs([double]$comparison.candidateMeanNs - 1600.0) -lt 0.001) 'Hosting candidate center is incorrect.'
    Assert-True ($null -eq $comparison.PSObject.Properties['timeDeltaPercent']) 'Hosting evolution output must not contain timeDeltaPercent.'
    Assert-True ($null -eq $comparison.PSObject.Properties['allocationDeltaPercent']) 'Hosting evolution output must not contain allocationDeltaPercent.'
