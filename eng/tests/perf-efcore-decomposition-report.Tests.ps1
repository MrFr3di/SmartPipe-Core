...
try {
    ...
    $alloc1 = @{
        ...
        CompiledPipelineSingle = 900.0
        RawHundredRows = 1000.0
        PipelineHundredRows = 3000.0
        CompiledPipelineHundredRows = 2500.0
    }
    ...
    Assert-True (@($normalized.records).Count -eq 12) 'Expected twelve raw EF decomposition records.'
    ...
    Assert-True ([Math]::Abs([double]$single.compiledPipelineMeanNs - 1850.0) -lt 0.001) 'Single compiled center is incorrect.'
    Assert-True ([Math]::Abs([double]$single.normalOverRawRatio - 2.0) -lt 0.001) 'Single Normal/Raw ratio is incorrect.'
    Assert-True ([Math]::Abs([double]$single.compiledOverRawRatio - (1850.0 / 1050.0)) -lt 0.001) 'Single Compiled/Raw ratio is incorrect.'
    Assert-True ([Math]::Abs([double]$single.compiledOverNormalRatio - (1850.0 / 2100.0)) -lt 0.001) 'Single Compiled/Normal ratio is incorrect.'
    ...
    Assert-True ([Math]::Abs([double]$hundred.compiledPipelineMeanNs - 13500.0) -lt 0.001) 'Hundred compiled center is incorrect.'

    $expectedNormalIncremental = (5000.0 - 1050.0) / 99.0
    $expectedCompiledIncremental = (3000.0 - 800.0) / 99.0
    ...
    Assert-True ($markdown.Contains('Raw EF, normal QuerySource and CompiledQuerySource')) 'Markdown must explain the three-way decomposition.'
