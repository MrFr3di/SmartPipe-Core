...
try {
    ...
    Assert-True ($output -contains 'PERF_HOSTING_REPORT_OK run=synthetic-healthchecks-report-test comparisons=2') 'HealthChecks report did not complete successfully.'
    ...
    Assert-True ($methods[0] -ceq 'ResolveHealthCheckService') 'ResolveHealthCheckService comparison is missing.'
    Assert-True ($methods[1] -ceq 'CheckRegisteredPipeline') 'CheckRegisteredPipeline comparison is missing.'
