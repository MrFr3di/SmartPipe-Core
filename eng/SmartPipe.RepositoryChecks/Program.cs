        ...
        try
        {
            ...
            switch (command)
            {
                ...
                case RunConsumersCommandOptions consumers:
                    var consumerResults = await new ConsumerScenarioRunner().RunAsync(new(consumers.RepositoryRoot, consumers.Set, consumers.PackageDirectory, consumers.PackageVersion, consumers.ManifestPath, consumers.Category, consumers.Scenario), cancellation.Token).ConfigureAwait(false);
