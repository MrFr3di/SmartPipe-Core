...
6. `SmartPipe.Extensions` перестаёт принимать новые реализации. В 2.2.0 он сохраняет type forwarders и замороженный legacy DI/Hosting/Health compatibility cluster, который невозможно разделить без breaking change; новые leaf packages никогда от него не зависят.
